(function (global, factory) {
   ...
   */
    ...
    const lodash_assignin = assignIn;

    // Message type used for dispatch events
    // from the Proxy Stores to background
    const DISPATCH_TYPE = 'chromex.dispatch'; // Message type for state update events from
    // background to Proxy Stores

    const STATE_TYPE = 'chromex.state'; // Message type for state patch events from
    // background to Proxy Stores

    const PATCH_STATE_TYPE = 'chromex.patch_state'; // The default name for the port communication via
    // webext-redux

    const DEFAULT_PORT_NAME = 'chromex.port_name';

    const noop = function noop(payload) {
        return payload;
    };

    const transformPayload = function transformPayload(message) {
        const transformer = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : noop;
        return _objectSpread({}, message, message.payload ? {
            payload: transformer(message.payload)
        } : {});
    };

    const deserializeListener = function deserializeListener(listener) {
        const deserializer = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : noop;
        const shouldDeserialize = arguments.length > 2 ? arguments[2] : undefined;
   ...
   */
    ...
    const withDeserializer = function withDeserializer() {
        const deserializer = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : noop;
   ...
   */
    ...
    const withSerializer = function withSerializer() {
        const serializer = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : noop;
        return function (sendMessageFn) {
            const messageArgIndex = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
            return function () {
    ...
    const DIFF_STATUS_UPDATED = 'updated'; // The `change` value for removed fields resulting from shallow diff

    const DIFF_STATUS_REMOVED = 'removed';

    function shallowDiff(obj, difference) {
        const newObj = { ...obj };
        difference.forEach((_ref) => {
            const change = _ref.change;
            const key = _ref.key;
  ...
  (function () {
     ...
     */
      function Store() {
          ...
          const _ref = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : defaultOpts;
          const _ref$portName = _ref.portName;
          const portName = _ref$portName === void 0 ? defaultOpts.portName : _ref$portName;
          const _ref$state = _ref.state;
          const state = _ref$state === void 0 ? defaultOpts.state : _ref$state;
          const _ref$extensionId = _ref.extensionId;
          const extensionId = _ref$extensionId === void 0 ? defaultOpts.extensionId : _ref$extensionId;
          const _ref$serializer = _ref.serializer;
          const serializer = _ref$serializer === void 0 ? defaultOpts.serializer : _ref$serializer;
          const _ref$deserializer = _ref.deserializer;
          const deserializer = _ref$deserializer === void 0 ? defaultOpts.deserializer : _ref$deserializer;
          const _ref$patchStrategy = _ref.patchStrategy;
          const patchStrategy = _ref$patchStrategy === void 0 ? defaultOpts.patchStrategy : _ref$patchStrategy;
          ...
          this.readyPromise = new Promise((resolve) => _this.readyResolve = resolve);
    ...
    */
      ...
      _createClass(Store, [{
          ...
          value: function ready() {
              const cb = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
       ...
       */
          ...
          value: function subscribe(listener) {
              ...
              return function () {
                  _this2.listeners = _this2.listeners.filter((l) => l !== listener);
              };
          }
          /**
          ...
          value: function dispatch(data) {
              ...
              return new Promise((resolve, reject) => {
                  _this3.serializedMessageSender(_this3.extensionId, {
                      ...
                      if (error) {
                          const bgErr = new Error(''.concat(backgroundErrPrefix).concat(error));
   ...
   */
    ...
    const promiseResponder = function promiseResponder(dispatchResult, send) {
        Promise.resolve(dispatchResult).then((res) => {
            send({
                error: null,
                value: res
            });
   ...
   */
    ...
    const wrapStore = (function (store) {
        const _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : defaultOpts$1;
        const _ref$portName = _ref.portName;
        const portName = _ref$portName === void 0 ? defaultOpts$1.portName : _ref$portName;
        const _ref$dispatchResponde = _ref.dispatchResponder;
        const dispatchResponder = _ref$dispatchResponde === void 0 ? defaultOpts$1.dispatchResponder : _ref$dispatchResponde;
        const _ref$serializer = _ref.serializer;
        const serializer = _ref$serializer === void 0 ? defaultOpts$1.serializer : _ref$serializer;
        const _ref$deserializer = _ref.deserializer;
        const deserializer = _ref$deserializer === void 0 ? defaultOpts$1.deserializer : _ref$deserializer;
        const _ref$diffStrategy = _ref.diffStrategy;
        const diffStrategy = _ref$diffStrategy === void 0 ? defaultOpts$1.diffStrategy : _ref$diffStrategy;
     ...
     */
        ...
        const dispatchResponse = function dispatchResponse(request, sender, sendResponse) {
            if (request.type === DISPATCH_TYPE && request.portName === portName) {
                const action = { ...request.payload, _sender: sender };
    ...
    */
        ...
        const connectState = function connectState(port) {
            if (port.name !== portName) {
                return;
            }

            const serializedMessagePoster = withSerializer(serializer)(function () {
                return port.postMessage.apply(port, arguments);
            });
            let prevState = store.getState();

            const patchState = function patchState() {
            ...
            const unsubscribe = store.subscribe(patchState); // when the port disconnects, unsubscribe the sendState listener
        ...
        const shouldDeserialize = function shouldDeserialize(request) {
